...
namespace pmn {
    ...
    [[nodiscard]] std::array<idx_t, METIS_NOPTIONS> createPartitionOptions() {
        ...
        options[METIS_OPTION_CONTIG] = 0; // 1 -> force contiguous partitions (I think that means that nodes in groups are connected? maybe not?)
    ...
    void create_dag(const std::vector<uint32_t> &indices, const std::vector<float> &vertices, size_t vertex_stride) {
        ...
        printf("meshlet graph: took %ld ms\n", std::chrono::duration_cast<std::chrono::milliseconds>(end_build_meshlet_graph - start_build_meshlet_graph).count());
        ...
        printf("graph partition: took %ld ms\n", std::chrono::duration_cast<std::chrono::milliseconds>(end_partition_meshlet_graph - start_partition_meshlet_graph).count());
